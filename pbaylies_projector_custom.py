...
def project(
    ...
    *,
    ...
    for step in range(num_steps):
        ...
        loss.backward()
        optimizer.step()
		#commented to reduce text on colab lol
...
@click.option('--img-name',            help='Image file name', default='img', show_default=True)
def run_projection(
    network_pkl: str,
    target_fname: str,
    target_text: str,
    ...
    PIL.Image.fromarray(synth_image, 'RGB').save(f'{outdir}/'+img_name+'_proj.png')
    np.savez(f'./data/'+img_name+'_proj_w.npz', w=projected_w.unsqueeze(0).cpu().numpy())
